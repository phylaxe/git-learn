...
setup:
  ...
  - cmd: "git commit -m 'Zweite Datei hinzugefuegt'"
  - cmd: "echo 'Dritte Datei' > datei3.txt"
  - cmd: "git add datei3.txt"
  - cmd: "git commit -m 'Dritte Datei hinzugefuegt'"
...
task: |
  ...
  `git log --oneline` fuer eine kompakte Ansicht aus.

hints:
  - "Mit `git log` siehst du alle Commits mit Details"
  - "Mit `git log --oneline` bekommst du eine kompakte Uebersicht"
  - "Der zweite Commit hat die Nachricht 'Zweite Datei hinzugefuegt'"
...
Fuer eine kompaktere Ansicht gibt es `git log --oneline`, das jeden
Commit in einer einzigen Zeile zusammenfasst.

### Was du lernst
- Wie du mit `git log` die Commit-Geschichte anzeigst
- Wie `git log --oneline` eine kompakte Uebersicht liefert
