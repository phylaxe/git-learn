...
task: |
  ...
  Aufgaben:
  1. Fuehre ps aux aus und speichere die komplette Ausgabe in "prozesse.txt".
     Beispiel: ps aux > prozesse.txt

  2. Zaehle wie viele Prozesse gerade laufen (ohne die Kopfzeile).
     Reiche die Anzahl mit check ein.
     Beispiel: ps aux | wc -l  (dann 1 abziehen fuer die Kopfzeile)
     Dann: check "42"  (mit der tatsaechlichen Zahl)

hints:
  - "ps aux zeigt alle laufenden Prozesse"
  - "ps aux > prozesse.txt speichert die Ausgabe in eine Datei"
  - "ps aux | wc -l zaehlt alle Zeilen inklusive Kopfzeile"
  - "Ziehe 1 ab fuer die Kopfzeile: ps aux | tail -n +2 | wc -l"
  - "Reiche die Anzahl ein: check \"42\" (mit deiner tatsaechlichen Zahl)"
...
ps aux          # Alle Prozesse aller Benutzer (am nuetzlichsten)
...
`htop` (falls installiert) ist eine modernere Alternative mit Farben und Maus-Unterstuetzung.
...
```bash
ls /proc/                     # Verzeichnisse fuer jeden Prozess
cat /proc/$$/status           # Status der aktuellen Shell
cat /proc/$$/cmdline          # Befehl der aktuellen Shell
ls /proc/1/                   # Informationen ueber Prozess 1 (init/systemd)
```

### Prozessstatus-Codes

- **R** (Running) – Laeuft gerade oder wartet auf CPU
- **S** (Sleeping) – Schlaeft und wartet auf Ereignis
