...
task: |
  Erstelle deinen ersten Commit! Füge zuerst README.md zur Staging Area
...
Ein Commit ist das Herzstück von Git. Er speichert einen Snapshot
aller gestagten Änderungen zusammen mit:
- Einer **Commit-Nachricht**, die beschreibt, was geändert wurde
- Einem **Autor** und **Zeitstempel**
- Einem **Hash** (eine eindeutige ID wie `a1b2c3d`)

Jeder Commit zeigt auf seinen Vorgänger -- so entsteht die
Geschichte deines Projekts.

### Was du lernst
- Wie du mit `git commit -m "nachricht"` einen Commit erstellst
- Was ein Commit enthält (Nachricht, Autor, Hash)
