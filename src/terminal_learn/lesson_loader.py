...
def load_all_lessons(
    lessons_dir: Path, level_order: list[str] | None = None
) -> list[Lesson]:
    """Load all lessons from subdirectories, sorted by level and order."""
    order = level_order or LEVEL_ORDER
    lessons: list[Lesson] = []
    for md_file in sorted(lessons_dir.rglob("*.md")):
        lessons.append(load_lesson(md_file))
    lessons.sort(key=lambda l: (order.index(l.level) if l.level in order else 99, l.order))
    return lessons
